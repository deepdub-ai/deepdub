...
def tts(ctx, text: str, voice_prompt_id: str, locale: str, model: str, temperature: float):
    ...
    with open(fname, "wb") as f:
        f.write(response)
    logger.info("TTS response saved to %s", fname)
...
@click.pass_context
def tts_async(ctx, text: str, voice_prompt_id: str, locale: str, model: str, format: str, sample_rate: int, headerless: bool):
    client = DeepdubClient(api_key=ctx.obj["api_key"])
